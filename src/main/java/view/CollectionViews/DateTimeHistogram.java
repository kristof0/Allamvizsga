...
public class DateTimeHistogram implements IGameLogic {
...
//HUD
    ...
    int[] offset;
    public DateTimeHistogram(IImageLoader textureLoader) {
    ...
    public DateTimeHistogram(IImageLoader textureLoader, int[] showedImages) {
        showed=new boolean[textureLoader.getNumberOfImages()];
    ...
    public DateTimeHistogram(IImageLoader textureLoader, TreeSet<Integer> showedImages) {
...
///
    final void  SetPosition(float[] positions,int nthPic,int nthPos,int column,int row){
        //float x=((oszlop%numberOfImagesPerRow)*pictureSize-pictureSize;//-1
        float x=(column)*pictureSize-1;//-1
        float y=(row+1)*pictureSize-1;//+pictureSize;
       ...
       // this.groupByTime=this.textureLoader.getGroupsByDate();
        ...
        for(;i<order.length;++i){
            ...
            if(showed[i] ) {
                ...
                while(c>-1){
                    col++;
                    c-=groupByTime.get(col);
                }
                row=groupByTime.get(col)+(c);
                if (marked[k]) {
                    ...
                    //SetPosition(markedPositions, markedIndex,order[i], eredetiMeret-tempGroupByTime.size(),q--);//markedindex
                    SetPosition(markedPositions, markedIndex,order[i], col,row);//markedindex
                    ...
                    //SetPosition(positions,nonMarkedIndex,order[i], eredetiMeret-tempGroupByTime.size(),q--);
                    SetPosition(positions,nonMarkedIndex,order[i], col,row);
...
/*            if(this.selectDetector.selectGameItem(window, mouseInput.getCurrentPos(),
                    camera,picturesPos,marked,pictureSize,numberOfImagesPerRow,invOrder.length,invOrder,mouseInput)){
                reorderImages();
            }*/
            ...
            //if(clickCord.x>groupByTime.size()*pictureSize){return;}
            ...
            int pictureIndex=0;
            if(this.groupByTime.get(tx)<ty){
                return;
            }else{
               // pictureIndex=invOrder[this.groupByTime.get(tx)+ty];


                int px=0;
                int i=0;
                while(i<tx){
                    px+=this.groupByTime.get(i);
                    i++;

                }
                px+=ty;
                pictureIndex=invOrder[px];
            }
...
///
        ...
        numberOfImagesPerRow=max(textureLoader.getMaxNumberOfRows(),textureLoader.getNumberOfCols());


        //numberOfImagesPerRow=(int)ceil(sqrt(showedImageNumber));
        pictureSize=2.0f/( numberOfImagesPerRow);
        System.out.println("Numberofimagesperrow "+numberOfImagesPerRow);
