...
public class CircularDateHistogram2D implements IGameLogic {
...
//HUD
    ...
    int[] offset;
    public CircularDateHistogram2D(IImageLoader textureLoader) {
    ...
    public CircularDateHistogram2D(IImageLoader textureLoader, int[] showedImages) {
        showed=new boolean[textureLoader.getNumberOfImages()];
    ...
    public CircularDateHistogram2D(IImageLoader textureLoader, TreeSet<Integer> showedImages) {
    ...
    final void  SetPosition(float[] positions,int nthPic,int column,int row){
        ...
        float x= (float) (Math.cos(angle) * (RADIUS+row*pictureSize*Math.sqrt(2)));
        float y= (float) (Math.sin(angle) * (RADIUS+row*pictureSize*Math.sqrt(2)));
        ...
        //System.out.println("nth="+nthPic+" j= "+j+" X= "+x+" Y="+y+" oszlop= "+oszlop+" magass= "+magassag);
...
///
        ...
        System.out.println("NumberOfImagesPerRow="+numberOfCols);
        System.out.println("MaxRows="+textureLoader.getMaxNumberOfRows());
        System.out.println("GroupByTime="+groupByTime.toString());
        System.out.println("NumberofCols="+textureLoader.getNumberOfCols());

       
        pictureSize=2.0f/( numberOfCols);
        RADIUS = (numberOfCols*pictureSize*Math.sqrt(2))/ (2*PI) ;
        while((textureLoader.getMaxNumberOfRows()*pictureSize+RADIUS)>1){
            pictureSize=3*pictureSize/4;
            RADIUS=(numberOfCols*pictureSize*Math.sqrt(2))/ (2*PI) ;
        }
