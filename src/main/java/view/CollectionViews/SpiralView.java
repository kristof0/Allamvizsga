...
public class SpiralView implements IGameLogic{
...
//HUD
    ...
    int[] offset;
    public SpiralView(IImageLoader textureLoader) {
    ...
    public SpiralView(IImageLoader textureLoader, int[] showedImages) {
    ...
    public SpiralView(IImageLoader textureLoader, TreeSet<Integer> showedImages) {
    ...
    final void  SetPosition(float[] positions,int nthPic,int nthPos){
       ...
       // double angle = Math.toRadians(((double) nthPos / numberOfImages) * 180d*sqrt(2));
        double r=sqrt(nthPos)/sqrt(numberOfImages);
        System.out.println("rrrrr "+r+" "+nthPos+" "+numberOfImages);
        double xb=r*cos(total_angle[nthPos] );
        double yb=r*sin(total_angle[nthPos] );
...
//--
            ...
            double dist=Math.sqrt((clickCord.y* clickCord.y  + clickCord.x * clickCord.x ));
            System.out.println("Distance to the center="+dist);

            System.out.println("cc:x="+clickCord.x+" y="+clickCord.y);
